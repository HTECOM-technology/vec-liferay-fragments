...
MODULE_DIR="$SCRIPT_DIR/journal-web"

# ─── Chạy trên server: build JAR và copy vào osgi/modules ───────────────────
if [ "$1" = "--server" ]; then
    DEPLOY_DIR="/root/vec/bundles/custom-modules"
...
echo ">>> Uploading custom-modules -> $SERVER:/root/vec/custom-modules/"
rsync -avz \
    --exclude='.git/' \
    ...
    --exclude='*.class' \
    -e "ssh $SSH_OPTS" \
    "$SCRIPT_DIR/" "$SERVER:/root/vec/custom-modules/"

echo ">>> Running build on server..."
ssh $SSH_OPTS \
    "$SERVER" \
    "bash -l -c 'chmod +x /root/vec/custom-modules/deploy.sh && /root/vec/custom-modules/deploy.sh --server'"
