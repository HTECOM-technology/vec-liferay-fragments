...
public class JournalDisplayContext {
	...
	public ResultRowSplitter getResultRowSplitter() {
		if (isCreateDateRangeFilterEnabled()) {
			return null;
		}
	...
	public String getCreateDateRangeEndDateString() {
		if (!isCreateDateRangeFilterEnabled()) {
			return StringPool.BLANK;
		}

		return _getCreateDateRange()[1];
	}

	public String getCreateDateRangeStartDateString() {
		if (!isCreateDateRangeFilterEnabled()) {
			return StringPool.BLANK;
		}

		return _getCreateDateRange()[0];
	}
	...
	public boolean isCreateDateRangeFilterEnabled() {
		return _getCreateDateRange() != null;
	}
	...
	private SearchContainer<?> _getArticlesSearchContainer()
		...
		if (!isCreateDateRangeFilterEnabled() && !hasAssetFilter() &&
	...
	private String[] _getCreateDateRange() {
		if (_createDateRangeChecked) {
			return _createDateRange;
		}

		_createDateRangeChecked = true;
		...
		if (!_isCreateDateString(endDateString) ||
			!_isCreateDateString(startDateString)) {
		...
		try {
			LocalDate endDate = LocalDate.parse(
				endDateString, DateTimeFormatter.ISO_LOCAL_DATE);
			LocalDate startDate = LocalDate.parse(
				startDateString, DateTimeFormatter.ISO_LOCAL_DATE);

			if (startDate.isAfter(endDate)) {
				return null;
			}

			_createDateRange = new String[] {startDateString, endDateString};
			_createDateRangeEndTime = endDate.plusDays(
				1
			).atStartOfDay(
			...
			_createDateRangeStartTime = startDate.atStartOfDay(
				_themeDisplay.getTimeZone(
				).toZoneId()
			).toInstant(
			).toEpochMilli();
	...
	private boolean _isCreateDateString(String dateString) {
	...
	private PortletURL _getBasePortletURL(String tab) {
		...
		if (isCreateDateRangeFilterEnabled()) {
			portletURL.setParameter(
				"endDate", getCreateDateRangeEndDateString());
			portletURL.setParameter(
				"startDate", getCreateDateRangeStartDateString());
		}
	...
	private void _setBooleanClauses(SearchContext searchContext) {
		...
		if (isCreateDateRangeFilterEnabled()) {
			booleanFilter.add(
				new RangeTermFilter(
					Field.CREATE_DATE, true, false,
					String.valueOf(_createDateRangeStartTime),
					String.valueOf(_createDateRangeEndTime)),
				BooleanClauseOccur.MUST);
		}
	...
	private String[] _createDateRange;
	private boolean _createDateRangeChecked;
	private long _createDateRangeEndTime;
	private long _createDateRangeStartTime;
