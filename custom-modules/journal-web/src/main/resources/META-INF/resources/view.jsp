...
<div class="align-items-end bg-light border-bottom d-flex flex-wrap px-3 py-2" id="<portlet:namespace />createDateRangeFilter">
	<div class="form-group form-group-sm mb-0 mr-2">
		<label class="mb-1" for="<portlet:namespace />startDate">
			<liferay-ui:message key="start-date" />
		</label>

		<input class="form-control form-control-sm" data-start-date id="<portlet:namespace />startDate" type="date" value="<%= journalDisplayContext.getCreateDateRangeStartDateString() %>" />
	</div>

	<div class="form-group form-group-sm mb-0 mr-2">
		<label class="mb-1" for="<portlet:namespace />endDate">
			<liferay-ui:message key="end-date" />
		</label>

		<input class="form-control form-control-sm" data-end-date id="<portlet:namespace />endDate" type="date" value="<%= journalDisplayContext.getCreateDateRangeEndDateString() %>" />
	</div>

	<button class="btn btn-primary btn-sm mr-2" data-apply-date-range type="button">
		<liferay-ui:message key="apply" />
	</button>

	<button class="btn btn-secondary btn-sm" data-clear-date-range type="button">
...
<liferay-frontend:component
	context='<%=
		HashMapBuilder.<String, Object>put(
			"formId", liferayPortletResponse.getNamespace() + "createDateRangeFilter"
		).put(
			"portletNamespace", liferayPortletResponse.getNamespace()
		).build()
	%>'
	module="{CreateDateRangeFilter} from journal-web"
/>
