...
public class GroupSettingsAuditModelListener extends BaseModelListener<Group> {

	@Override
	public void onAfterUpdate(Group originalGroup, Group group) {
		String beforeData = _auditSnapshotService.snapshotGroup(originalGroup);
		String afterData = _auditSnapshotService.snapshotGroup(group);
		String classPK = String.valueOf(group.getGroupId());
		String targetTitle = "Site Settings: " + group.getFriendlyURL();

		long auditLogId = _auditLogService.startPending(
			AuditActionType.UPDATE, AuditTargetType.SETTING,
			Group.class.getName(), classPK, targetTitle, null, beforeData, null,
			null, null, null,
