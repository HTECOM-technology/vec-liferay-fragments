...
- **Cầu dao chống vòng lặp**: chỉ khi **cùng một path** bị chuyển hướng sang đăng nhập 3 lần liên tiếp mà phiên vẫn chưa chạm được giao diện đăng nhập, filter cho đúng request đó đi tiếp, ghi `WARN` (kèm `host`, `referer`, `userAgent`) rồi đặt lại bộ đếm. Mở sang path khác thì bộ đếm bắt đầu lại, nên crawler/bot giữ cookie nhưng không theo redirect không lọt qua được. Bản cũ đếm theo session và không đặt lại, một phiên như vậy từng đi được mọi trang không cần đăng nhập. Bộ đếm được xoá khi chạm trang đăng nhập hoặc khi đã đăng nhập.
