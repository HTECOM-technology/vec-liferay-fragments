...
- Domain công khai (xem `domainpolicy`) → `domain-policy/public-domain.css` để ẩn lối vào đăng nhập.
...
bash build-custom-bundles.sh --shell
# Trong container:
bash custom-bundles/admin-ui/test-domain-policy.sh
```
...
- **Cầu dao chống vòng lặp**: chỉ khi **cùng một URL** (path + query) bị chuyển hướng sang đăng nhập 3 lần liên tiếp mà phiên vẫn chưa chạm được giao diện đăng nhập, filter cho đúng request đó đi tiếp, ghi `WARN` (kèm `host`, `referer`, `userAgent`) rồi đặt lại bộ đếm. Mở sang URL khác (kể cả cùng path nhưng query `redirect=` lồng nhau khác nhau) thì bộ đếm bắt đầu lại; trang intranet (`isIntranetPath`) không bao giờ mở cầu dao vì đang là public layout, nên crawler/bot giữ cookie nhưng không theo redirect không lọt qua được. Bản cũ đếm theo session và không đặt lại, một phiên như vậy từng đi được mọi trang không cần đăng nhập. Bộ đếm được xoá khi chạm trang đăng nhập hoặc khi đã đăng nhập.
...
- Khi component kích hoạt, filter ghi log `INFO`: `VEC Domain Access Policy Filter activated: ...` — chỉ chứng minh module đã deploy, **không** chứng minh filter nằm trong chain.
- 50 request điều hướng trang đầu tiên sau mỗi lần activate được ghi `INFO` dạng `Domain access policy: host=..., role=..., path=..., signedIn=..., handled=...`. Đây mới là bằng chứng filter thực sự chạy. Hết hạn mức thì bật `DEBUG` cho `vn.vec.custom.admin.domainpolicy` để xem tiếp.
...
| `domain-policy/public-domain.css` | Ẩn lối vào đăng nhập trên domain công khai (xem package `domainpolicy`) |
