...
// API Web Content Structure
const API_URL =
    "https://portal.tctvec.vn/o/headless-delivery/v1.0/content-structures/1266959/structured-contents?pageSize=100";
...
function getSelectDisplay(field) {
  return field?.contentFieldValue?.data || "";
}

function getSelectValue(field) {
  return field?.contentFieldValue?.value || "";
}

function mapStructuredItem(item) {
  const groupsField = (item.contentFields || []).find(
      (f) => f.name === "groups"
  );

  const nested = groupsField?.nestedContentFields || [];

  const typeField = nested.find((f) => f.name === "type");
  const subGroupFields = nested.filter((f) => f.name === "subGroups");

  return {
    title: normalizeText(getSelectDisplay(typeField)),
    type: normalizeText(getSelectValue(typeField)).toLowerCase(),
    columns: subGroupFields
        .map((sub) => {
          const subNested = sub.nestedContentFields || [];

          const subtitleField = subNested.find((f) => f.name === "subtitle");
          const contentFields = subNested.filter((f) => f.name === "content");

          return {
            title: normalizeText(getFieldValue(subtitleField)),
            items: contentFields
                .map((f) => normalizeText(getFieldValue(f)))
                .filter(Boolean),
          };
        })
...
function transformApiData(apiData) {
  const mapped = (apiData.items || []).map(mapStructuredItem);

  const tgd = mapped.find((item) => item.type === "btgd") || null;

  const right = mapped
      .filter((item) => item.type !== "btgd")
      .map((item) => ({
        title: item.title,
        type: item.type,
      }));
  ...
  return {
    tgd: tgd ? { title: tgd.title } : { title: "" },
    columns: tgd ? tgd.columns : [],
    right,
  };
}
...
function NodeBox({ label, members, type = "white", children, onClick }) {
  return (
      ...
      >
        <NodeLabel dangerouslySetInnerHTML={{ __html: toHtml(label) }} />
...
const SoDoToChuc = () => {
  ...
  useEffect(() => {
    ...
    async function fetchOrgChart() {
      try {
        ...
        const mapped = transformApiData(json);

        if (mounted) {
          setOrgData(mapped);
        }
      } catch (err) {
        console.error("Load sơ đồ tổ chức lỗi:", err);

        if (mounted) {
          setError("Không tải được dữ liệu sơ đồ tổ chức.");
        }
      } finally {
        if (mounted) {
          setLoading(false);
  ...
  if (loading) {
    return <LoadingText>Đang tải sơ đồ tổ chức...</LoadingText>;
  }
  ...
  return (
      <ChartWrap>
        <HoiDongThanhVienModal
            open={hoiDongModalOpen}
            onClose={() => setHoiDongModalOpen(false)}
        />

        {/* Hàng 1: Hội đồng thành viên + các ban bên phải */}
        <Row justify="center">
          <Col span={8} style={{ display: "flex", justifyContent: "center" }}>
            <NodeBox
                label="HỘI ĐỒNG THÀNH VIÊN"
                type="red"
                onClick={() => setHoiDongModalOpen(true)}
            >
              {orgData.right.length > 0 && (
        ...
        <MiddleRow>
          <TopLeftCol>
            ...
            <NodeBox label={orgData.tgd?.title || "BAN TỔNG GIÁM ĐỐC"} type="red" />
          </TopLeftCol>
        </MiddleRow>

        {/* Hàng 3: các khối / ban / đơn vị */}
        <MiddleRow>
          <BranchConnectorWrap>
            <svg
                width="684"
            ...
            <ThreeColumns>
              {orgData.columns.map((col, index) => (
                  <Column key={`${col.title}-${index}`}>
                    <ColumnHeader>
                      <NodeBox label={col.title} type="outline" />
                    </ColumnHeader>
                    ...
                    >
                      <path
                          d="M4.7998 0.75C4.7998 0.335786 4.46402 0 4.0498 0C3.63559 0 3.2998 0.335786 3.2998 0.75H4.0498H4.7998ZM3.5498 17.5801C3.8427 17.873 4.25691 17.873 4.5498 17.5801L7.87993 14.25C8.17282 13.9571 8.17282 12.5429 7.87993 12.25L4.0498 8.41987L0.219678 12.25C-0.0732155 12.5429 -0.0732155 13.9571 0.219678 14.25L3.5498 17.5801ZM4.0498 0.75H3.2998V13.75H4.0498H4.7998V0.75H4.0498Z"
                          fill="#D9D9D9"
                      />
                    </svg>
                    ...
                    <ColumnList>
                      {(col.items || []).map((item, idx) => (
                          <NodeBox key={`${item}-${idx}`} label={item} />
                      ))}
                    </ColumnList>
                  </Column>
              ))}
