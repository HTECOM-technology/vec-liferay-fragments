import React from "react";
import PropTypes from "prop-types";
import {
    SalaryCard as StyledSalaryCard,
    CardHeader,
    CardBody,
    CardRow,
} from "./style";

const SalaryIcon = () => (
    <svg
        width="24"
        height="24"
        viewBox="0 0 24 24"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
    >
        <path
            d="M10.7273 22H12.5C15.7875 22 17.4312 22 18.5376 21.092C18.7401 20.9258 18.9258 20.7401 19.092 20.5376C20 19.4312 20 17.7875 20 14.5V7.81818C20 6.12494 20 5.27832 19.732 4.60214C19.3012 3.5151 18.3902 2.65765 17.2352 2.2522C16.5168 2 15.6173 2 13.8182 2C10.6698 2 9.09563 2 7.83836 2.44135C5.81714 3.15088 4.22282 4.65142 3.46894 6.55375C3 7.73706 3 9.21865 3 12.1818L3 14.7273C3 17.7966 3 19.3313 3.8477 20.3971C4.09058 20.7025 4.37862 20.9736 4.70307 21.2022C5.83546 22 7.46607 22 10.7273 22Z"
            fill="#0090CF"
            fillOpacity="0.2"
        />
        <path
            d="M6.33333 8.66667C4.49238 8.66667 3 10.159 3 12C3 9.6 2.72475 7.54607 4.11173 5.33333C6.20111 2 10.9078 1.72222 13 2C11.1591 2 9.66667 3.49238 9.66667 5.33333C9.66667 5.99912 9.78333 6.78404 9.60988 7.43136C9.45576 8.00652 9.00652 8.45576 8.43137 8.60988C7.78404 8.78333 6.99912 8.66667 6.33333 8.66667Z"
            fill="white"
        />
        <path
            d="M12.5 22L10.7273 22C7.46607 22 5.83546 22 4.70307 21.2022C4.37862 20.9736 4.09058 20.7025 3.8477 20.3971C3 19.3313 3 17.7966 3 14.7273L3 12.1818C3 9.21865 3 7.73706 3.46894 6.55375C4.22282 4.65142 5.81714 3.15088 7.83836 2.44135C9.09563 2 10.6698 2 13.8182 2C15.6173 2 16.5168 2 17.2352 2.2522C18.3902 2.65765 19.3013 3.5151 19.732 4.60214C20 5.27832 20 6.12494 20 7.81818L20 10.5"
            stroke="#0090CF"
            strokeWidth="1.5"
            strokeLinecap="round"
            strokeLinejoin="round"
        />
        <path
            d="M3 12C3 10.159 4.49238 8.66667 6.33333 8.66667C6.99912 8.66667 7.78404 8.78333 8.43137 8.60988C9.00652 8.45576 9.45576 8.00652 9.60988 7.43136C9.78333 6.78404 9.66667 5.99912 9.66667 5.33333C9.66667 3.49238 11.1591 2 13 2"
            stroke="#0090CF"
            strokeWidth="1.5"
            strokeLinecap="round"
            strokeLinejoin="round"
        />
        <path
            d="M21 20.4923C20.5216 21.3957 19.6512 22 18.6568 22C17.147 22 15.9231 20.6071 15.9231 18.8889V17.1111C15.9231 15.3929 17.147 14 18.6568 14C19.6512 14 20.5216 14.6043 21 15.5077M15 18H18.9231"
            stroke="#0090CF"
            strokeWidth="1.5"
            strokeLinecap="round"
        />
    </svg>
);

function SalaryCard({ data, onClick }) {
    return (
        <StyledSalaryCard onClick={() => onClick && onClick(data)}>
            <CardHeader>
                <div className="header-icon">
                    <SalaryIcon />
                </div>
                <span className="header-title">Phiếu lương tháng {data.kyLuong}</span>
            </CardHeader>
            <CardBody>
                <CardRow>
                    <span className="row-label">Họ và tên</span>
                    <span className="row-value">{data.hoTen}</span>
                </CardRow>
                <CardRow>
                    <span className="row-label">Mã nhân viên</span>
                    <span className="row-value">{data.maNV}</span>
                </CardRow>
                <CardRow>
                    <span className="row-label">Kỳ lương</span>
                    <span className="row-value">{data.kyLuong}</span>
                </CardRow>
                <CardRow>
                    <span className="row-label">Chức vụ</span>
                    <span className="row-value">{data.chucVu}</span>
                </CardRow>
                <CardRow>
                    <span className="row-label">Phòng ban</span>
                    <span className="row-value">{data.phongBan}</span>
                </CardRow>
            </CardBody>
        </StyledSalaryCard>
    );
}

SalaryCard.propTypes = {
    data: PropTypes.object.isRequired,
    onClick: PropTypes.func,
};

SalaryCard.defaultProps = {
    onClick: () => {},
};

export default SalaryCard;
