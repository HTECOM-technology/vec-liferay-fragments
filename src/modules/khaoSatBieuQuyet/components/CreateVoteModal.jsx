...
import { Modal, Input, Select, Checkbox, DatePicker, TimePicker } from "antd";
...
const AddOptionButton = styled.button`
    ...
    margin-top: 8px;
    background: rgba(229, 247, 255, 1);
    border: 1px dashed rgba(0, 144, 207, 0.5);
    border-radius: 4px;
...
const MultipleChoiceSection = styled.div`
    display: flex;
    align-items: center;
    justify-content: space-between;
    margin-bottom: 20px;
...
const MultipleChoiceLabel = styled.span`
    font-size: 14px;
    color: rgba(30, 30, 30, 1);
`;
...
const SwitchWrapper = styled.label`
    ...
    input:checked + .slider {
        background-color: rgba(0, 144, 207, 1);
    }
...
function CreateVoteModal({ visible, onClose, onSubmit, submitting, initialData }) {
    // Form state
    ...
    useEffect(() => {
        ...
        const allParticipant = participants.some((item) => item.scopeType === "ALL");
        const organization = participants.find((item) => item.scopeType === "ORGANIZATION");
        const department = participants.find((item) => item.scopeType === "DEPARTMENT");
        ...
        setSelectAll(allParticipant);
        setSelectedUnit(organization?.organizationId || null);
        setSelectedDepartment(department?.departmentId || null);
        setSelectedMembers(users);
        setHasDeadline(Boolean(parsedStart || parsedEnd));
        setStartDate(parsedStart);
        setStartTime(parsedStart);
        setEndDate(parsedEnd);
        setEndTime(parsedEnd);
        setAllowMultiple(Boolean(initialData.allowMultiple));
    }, [visible, initialData]);
    ...
    useEffect(() => {
        ...
        getOrganizations({ parentOrganizationId: 0 })
            .then(setUnitOptions)
    ...
    useEffect(() => {
        if (!visible || selectAll || !selectedUnit) {
            setDepartmentOptions([]);
            return;
        }

        setLoadingDepartments(true);
        getOrganizations({ parentOrganizationId: selectedUnit })
            .then(setDepartmentOptions)
            .catch(() => setDepartmentOptions([]))
            .finally(() => setLoadingDepartments(false));
    }, [visible, selectAll, selectedUnit]);
    ...
    }, [visible, selectAll, selectedUnit, selectedDepartment]);
    ...
    const isFormValid = title.trim() !== "" && options.filter((opt) => opt.trim() !== "").length >= 2;
    ...
    return (
        ...
        >
            <ModalHeader>
                ...
                <span className="header-title">{initialData ? "Chỉnh sửa cuộc bình chọn" : "Tạo cuộc bình chọn"}</span>
            ...
            <ModalBody>
                ...
                <ParticipantSection>
                    <ParticipantHeader>
                        <FormLabel style={{ marginBottom: 0 }}>Người tham gia</FormLabel>
                        <CheckboxWrapper>
                    ...
                    <SelectRow>
                        ...
                        <StyledSelect
                            ...
                            disabled={selectAll || !selectedUnit}
                ...
                {/* Đặt thời hạn Section */}
                <DeadlineSection>
                    <DeadlineHeader>
                        <Checkbox checked={hasDeadline} onChange={(e) => setHasDeadline(e.target.checked)}>
                            Đặt thời hạn
                        </Checkbox>
                    </DeadlineHeader>
                ...
                <MultipleChoiceSection>
                    <MultipleChoiceLabel>Cho phép chọn nhiều phương án</MultipleChoiceLabel>
                    <SwitchWrapper>
                        <input
                            type="checkbox"
                ...
                <SubmitButton onClick={handleSubmit} disabled={!isFormValid || submitting}>
                    {submitting ? "Đang lưu..." : initialData ? "Lưu" : "Tạo"}
                </SubmitButton>
            </ModalBody>
        </Modal>
    );
